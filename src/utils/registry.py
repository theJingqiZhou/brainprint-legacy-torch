...
def get_module(parent, name):
    assert (
        name in _all_modules[parent]
    ), "{} is not found in {} registry, all supported names: {}".format(
        name, parent, list(_all_modules[parent].keys())
    )
...
def load_modules(init_file_path, root_module_name):
    ...
    module_paths = [
        f
        for f in glob.glob(os.path.join(current_directory, "**/*.py"), recursive=True)
